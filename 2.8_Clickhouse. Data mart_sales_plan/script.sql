...
WITH sales AS (SELECT shop_id, product_id, toYear(date) AS year, 
                      toMonth(date) AS month, SUM(sales_cnt) AS sales
               FROM  shop_dns sd
               GROUP BY 1,2,3,4
               UNION ALL
               SELECT shop_id, product_id, toYear(date) AS year, 
                      toMonth(date) AS month, sum(sales_cnt) AS sales
               FROM shop_mvideo sd
               GROUP BY 1,2,3,4
               UNION ALL
...
WHERE sl.month = toMonth(plan_date) AND sl.year = toYear(plan_date) AND month =6 AND year =2023
