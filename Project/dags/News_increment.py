...
#Загружаем библиотеки для Python оператора
...
#Парсим данные с сайта Ленты
 ...
 yesterday = (datetime.datetime.now()).strftime("%a, %d %b %Y ")

 names = soup.find_all('item')
 for name in names:
  dates=name.pubDate.text
  dates=str(dates)
 ...
 news_lenta['date'] = pd.to_datetime(news_lenta['date'])
...
#Загружаем датафрейм с новостями Ленты в БД в сырой слой данных
 ...
 news_lenta.to_sql('news_lenta', engine, index=False, if_exists= 'append')
...
#Парсим данные с сайта Тасс
 ...
 news_tass['date'] = pd.to_datetime(news_tass['date'])
 
#Загружаем датафрейм с новостями Ленты в БД в сырой слой данных
#Загружаем датафрейм с новостями Тасс в БД в сырой слой данных
 engine = create_engine('postgresql://postgres:password@host.docker.internal:5430/test')
 news_tass.to_sql('news_tass', engine, index=False, if_exists= 'append')
...
#Парсим данные с сайта Ведомости
 ...
 news_vedomosti['date'] = pd.to_datetime(news_vedomosti['date'])
#Загружаем датафрейм с новостями Ведомостей в БД в сырой слой данных
 engine = create_engine('postgresql://postgres:password@host.docker.internal:5430/test')
 news_vedomosti.to_sql('news_vedomosti', engine, index=False, if_exists= 'append')

"""
#Функция создания core слоя в БД
"""
def news_update_core():
...
# (как будут объединяться категории нужно уточнять у аналитиков    
    ...
    cursor.execute("""DELETE FROM categories USING categories b 
...
#Cоздаем витрину    
    cursor = conn.cursor()
    cursor.execute("DROP TABLE IF EXISTS public.vitrina_news;")
    cursor.execute("""CREATE TABLE public.vitrina_news 
...
#определяем даг
with DAG(dag_id = "News_increment", schedule_interval = "55 23 * * * ",
   default_args = default_args, tags=["1T", "test"], catchup = False) as dag:
        

   import_lenta = PythonOperator(task_id = "import_news_lenta_increment",
                                                python_callable = import_news_lenta)
   
   import_tass = PythonOperator(task_id = "import_news_tass_increment",
                                                python_callable = import_news_tass)
   
   import_vedomosti = PythonOperator(task_id = "import_news_vedomosti_increment",
                                                python_callable = import_news_vedomosti)
   
   news_core = PythonOperator(task_id = "news_update_core_increment",
                                                python_callable = news_update_core)
   news_vitrina = PythonOperator(task_id = "news_update_vitrina_increment",
                                                python_callable = news_update_vitrina)
